...
function applyFilters(cars){
  ...
  if(state.q){
    ...
    res = res.filter(c => `${c.brand} ${c.model} ${c.color}`.toLowerCase().includes(q));
  }
...
function render(){
  ...
  for(const c of cars){
    ...
    const link = node.querySelector('.car__thumb__wrap');
    const img = node.querySelector('.car__thumb');
    ...
    node.querySelector('.chip.year').textContent = c.year;
    node.querySelector('.chip.km').textContent = formatKm(c.km);
    node.querySelector('.chip.fuel').textContent = c.fuel;
    img.src = (c.images?.[0]) || 'assets/img/placeholder.jpg';
