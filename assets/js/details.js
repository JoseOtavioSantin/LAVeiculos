...
if(!car || !car.published){
  ...
  const featsEl = by('#features');
  const feats = Array.isArray(car.features) ? car.features : [];
  featsEl.innerHTML = ''; feats.forEach(name => { const li = document.createElement('li'); li.textContent = name; featsEl.appendChild(li); });

  const contactsEl = by('#contacts');
  const contacts = Array.isArray(car.consultants) && car.consultants.length ? car.consultants.filter(c => c.phone) : (car.phone ? [{ name:'', phone:car.phone }] : []);
  contacts.forEach(c => { const a = document.createElement('a'); a.className = 'btn primary'; a.textContent = c.name ? ('Falar com ' + c.name) : 'Falar no WhatsApp'; a.target = '_blank'; a.rel = 'noopener'; a.href = `https://wa.me/${c.phone}?text=${encodeURIComponent('Tenho interesse no '+car.brand+' '+car.model)}`; contactsEl.appendChild(a); });
  ...
  photoMain.src = imgs[0]; thumbs.innerHTML = '';
  imgs.forEach((src, i) => { const img = new Image(); img.src = src; img.alt = 'Foto '+(i+1); if(i===0) img.classList.add('active'); img.addEventListener('click', () => { all('.gallery__thumbs img').forEach(t => t.classList.remove('active')); img.classList.add('active'); photoMain.src = src; }); thumbs.appendChild(img); });
}